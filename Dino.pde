class Dino extends GameObject implements Comparable<Dino>{
  ...
  void update_brain_inputs(float[] next_obstacle_info, int speed){
    brain_inputs[0] = next_obstacle_info[0] / 900;                  // distancia normalizada
    brain_inputs[1] = (next_obstacle_info[1] - 450) / (1350 - 450); // posición x del obstáculo normalizada
    brain_inputs[2] = (next_obstacle_info[2] - 370) / (480 - 370);  // posición y del obstáculo normalizada
    brain_inputs[3] = (next_obstacle_info[3] - 30) / (146 - 30);    // ancho del obstáculo normalizado
    brain_inputs[4] = (next_obstacle_info[4] - 40) / (96 - 40);     // altura del obstáculo normalizada
    brain_inputs[5] = (y_pos - 278) / (484-278);                    // posición y del dino normalizada
    brain_inputs[6] = (speed - 15) / (30 - 15);                     // velocidad normalizada
