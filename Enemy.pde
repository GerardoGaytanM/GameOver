abstract class Enemy extends GameObject {
  ...
  Enemy() {
    x_pos = 1350; // Inicializar la posición x fuera de la pantalla
  }
  
  void update(int speed) {
    x_pos -= speed; // Actualizar la posición x basándose en la velocidad
  }
  
  boolean is_offscreen() {
    return x_pos + obj_width < 0; // Verificar si el obstáculo está fuera de la pantalla
  }
}
